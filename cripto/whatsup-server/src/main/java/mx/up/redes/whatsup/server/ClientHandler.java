...
//        this.registry = registry;
    ...
    private boolean processRegistration() throws IOException {
        ...
        if (!ControlUsuarios.getInstance().registerUser(username, this)) {
        ...
        ControlUsuarios.getInstance().broadcastUserList();
        return true;
    }
    ...
    private void handleListCommand() {
        ...
        String userList = ControlUsuarios.getInstance().getUserListCsv();
    ...
    private void handleSendCommand(String[] parts) {
        ...
        boolean sent = ControlUsuarios.getInstance().sendMessage(username, recipient, messageText);
        if (!sent) {
    ...
    private void cleanup() {
        try {
            if (username != null) {
                ControlUsuarios.getInstance().unregisterUser(username);
                ControlUsuarios.getInstance().broadcastUserList();
